...
var navUrl = ["#Home","about.html","people.html","products.html","#Board","contact.html"];
...
$(document).ready(function(){
    ...
    (function($){
        $(window).on("load",function(){
            $(".about-container").mCustomScrollbar({
                theme:"minimal"
            });
...
function createWorkElements(){
    ...
    for(var i = 0; i < Title.length; i++) {
        ...
        $('.inner-pb').append(
            ...
            .append(
                $('<div/>', {'class': 'pb-wrapper','style': 'background-image:url("'+newImages[0]+'")'})
                .append(
...
function createCaseStudy(){
    ...
    $(".csd-pic").each(function(){
        $(this).css("background-image",'url(' + csImages[k] + ')');
        $(this).css({opacity:0});
...
function populateWorkBg(){
    var k = 0;
    $(".bg-item-container").each(function(){
        var bgImages = Images[k].split(" ");
        $(this).css("background-image",'url(' + bgImages[0] + ')');
        $(this).attr("data-wname",Title[k]);
        $(this).attr("data-wloc",Loc[k]);
        k++;
    });
}
